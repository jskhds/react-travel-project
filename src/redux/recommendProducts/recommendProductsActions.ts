...
// thunk 支持的返回函数类型, 这个返回类型叫 ThunkAction，有四个参数，然后第二个参数有两个参数 dispatch 和 getState
export const giveMeDataActionCreator =
  (): ThunkAction<void, RootState, unknown, RecommendProductAction> =>
    async (dispatch, getState) => {
        dispatch(fetchRecommendProductStartActionCreator())
        try {
          const response = await axios.get("http://123.56.149.216:8089/api/productCollections") 
